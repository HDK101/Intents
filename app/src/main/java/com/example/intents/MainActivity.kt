...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        amb.entrarParametroBt.setOnClickListener {
            parl.launch(
                Intent("WINDOW_PARAMETRO").putExtra(
                    PARAMETRO_EXTRA,
                    amb.parametroTv.text.toString()
                )
            )
        }
        ...
        return when (item.itemId) {
            R.id.viewMi -> {
                startActivity(
                    Intent(
                        Intent.ACTION_VIEW,
                        Uri.parse(amb.parametroTv.text.toString())
                    )
                )
                true
            }
            R.id.callMi -> {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    if (checkSelfPermission(CALL_PHONE) == PERMISSION_GRANTED)
                        chamarNumero(true)
                    else permissaoChamadaArl.launch(CALL_PHONE)
                } else
                    chamarNumero(true)
                true
            }
            ...
            R.id.chooserMi -> {
                startActivity(
                    Intent(ACTION_CHOOSER).putExtra(EXTRA_TITLE, "Escolha seu navegadro preferido")
                        .putExtra(
                            EXTRA_INTENT,
                            Intent(ACTION_VIEW, Uri.parse(amb.parametroTv.text.toString()))
                        )
                )
                true
            }
    ...
    private fun chamarNumero(chamar: Boolean) {
        startActivity(
            Intent(
                ...
                Uri.parse("tel: ${amb.parametroTv.text}")
            )
        )
    }
